...
import { Calendar } from '@/components/ui/calendar'; // Assuming Calendar component is needed
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'; // Assuming Popover is needed
...
// Mock data - Exported to be accessible by Trips.tsx
...
export const Expenses: React.FC<{ tripId?: string; tripName?: string; }> = ({ tripId, tripName }) => {
    ...
    const confirmAndSaveExpense = () => {
        ...
        initialExpenses.push(expenseToConfirm); // Add to global mock data
    ...
    const handleEditExpense = (e: React.FormEvent) => {
        ...
        const updatedExpense: Expense = {
            ...
            value: valueNumber,
            expenseDate: expenseDate.toISOString(),
            expenseType, // Include expenseType in update
        };

        const index = initialExpenses.findIndex(e => e.id === currentExpense.id);
        if (index !== -1) {
            initialExpenses[index] = updatedExpense; // Update global mock data
        }
    ...
    const handleDeleteExpense = (expenseId: string) => {
        ...
        if (index !== -1) {
            initialExpenses.splice(index, 1); // Remove from global mock data
        }
    ...
    const openEditModal = (expense: Expense) => {
        ...
        try {
            ...
            setExpenseDate(undefined); // Handle invalid date string
        }
    ...
    const closeConfirmModal = () => {
        setIsConfirmModalOpen(false);
        setExpenseToConfirm(null);
        // Optionally re-open create modal
                        ...
                        <DialogContent className="sm:max-w-lg">
                            ...
                            <form onSubmit={handlePrepareExpenseForConfirmation} className="grid gap-4 py-4">
                                <div className="space-y-2">
                                    ...
                                    <Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} required placeholder="Ex: Pedágio, Alimentação..." />
                                </div>
                                <div className="space-y-2">
                ...
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        ...
                        <ul className="mt-3 list-disc list-inside space-y-1 text-sm text-foreground">
                            ...
                            <li><strong>Data da Despesa:</strong> {expenseToConfirm?.expenseDate ? formatDate(expenseToConfirm?.expenseDate) : 'N/A'}</li>
                        </ul>
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    ...
                    <Card key={expense.id} className="shadow-sm transition-shadow hover:shadow-md bg-card border border-border">
                        <CardHeader>
                            <div className="flex justify-between items-start"> {/* Changed items-center to items-start */}
                                <div>
                                    <CardTitle className="text-lg">{expense.description}</CardTitle> {/* Changed to text-lg for consistency */}
                                    <CardDescription>
                                        {formatCurrency(expense.value)} - {expense.expenseType}
                                    </CardDescription>
                                </div>
                        ...
                        <CardContent className="space-y-2 text-sm pt-4"> {/* Added pt-4 for spacing */}
                            ...
                            <div className="flex items-center gap-2">
                             ...
                             <div className="flex items-center gap-2">
                                ...
                                <span>Tipo: {expense.expenseType}</span>
                            </div>
                            {/* Removed redundant description display here */}
                        </CardContent>
                    </Card>
                ))}
            </div>
