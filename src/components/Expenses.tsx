...
import { PlusCircle, Eye, Edit, Trash2, Paperclip, Camera, Utensils, Hotel, Car, Wallet } from 'lucide-react'; // Added Wallet
...
const initialExpenses: Expense[] = [
  { id: 'e1', type: 'alimentacao', value: 55.50, description: 'Almoço posto X', timestamp: new Date(2024, 6, 21, 12, 30).toISOString() },
  { id: 'e2', type: 'pedagio', value: 12.80, description: 'Pedágio BR-116 km 50', timestamp: new Date(2024, 6, 21, 15, 0).toISOString(), attachment: 'https://via.placeholder.com/100x50.png?text=ReciboPedagio' , attachmentPreview: 'https://via.placeholder.com/100x50.png?text=ReciboPedagio'},
  { id: 'e3', type: 'hotel', value: 180.00, description: 'Pernoite Hotel Central', timestamp: new Date(2024, 6, 22, 8, 0).toISOString() },
];
...
export const Expenses: React.FC = () => {
  ...
  const [attachmentPreview, setAttachmentPreview] = useState<string | null>(null); // For image previews
   ...
   useEffect(() => {
      ...
      setExpenses(currentExpenses => [...currentExpenses].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
    }, []);
  ...
  const handleCreateExpense = (e: React.FormEvent) => {
    ...
    // For now, we just add it with the File object / preview URL
  ...
  const resetForm = () => {
   ...
   const getTypeIcon = (type: Expense['type']) => {
       const IconComponent = expenseTypeDetails[type]?.icon || Wallet; // Default to Wallet
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold">Registro de Despesas</h2>
             ...
             <form onSubmit={handleCreateExpense} className="grid gap-4 py-4">
                <div className="space-y-2">
                 ...
                 <div className="space-y-2">
                    ...
                    <Input id="expenseValue" type="number" value={expenseValue} onChange={(e) => setExpenseValue(Number(e.target.value) >= 0 ? Number(e.target.value) : '')} required placeholder="0,00" step="0.01" min="0" />
                 </div>
      ...
      {expenses.length === 0 ? (
        ...
        <div className="grid gap-4">
          {expenses.map((expense) => (
            <Card key={expense.id} className="shadow-sm transition-shadow hover:shadow-md">
              <CardHeader>
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        {expense.attachment && (
                            <Button variant="ghost" size="icon" asChild className="text-muted-foreground hover:text-primary h-8 w-8">
                               <a href={typeof expense.attachment === 'string' ? expense.attachment : '#'} target="_blank" rel="noopener noreferrer" title="Ver Anexo">
                       ...
                       <Dialog open={isEditModalOpen && currentExpense?.id === expense.id} onOpenChange={(isOpen) => !isOpen && closeEditModal()}>
                         <DialogTrigger asChild>
                           <Button variant="ghost" size="icon" onClick={() => openEditModal(expense)} className="text-muted-foreground hover:text-accent h-8 w-8">
                           ...
                           <DialogContent className="sm:max-w-md">
                             ...
                             <form onSubmit={handleEditExpense} className="grid gap-4 py-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="editExpenseValue" type="number" value={expenseValue} onChange={(e) => setExpenseValue(Number(e.target.value) >= 0 ? Number(e.target.value) : '')} required step="0.01" min="0" />
                                </div>
                                <div className="space-y-2">
                                 ...
                                 <div className="space-y-2">
                                      ...
                                      {!attachment && !attachmentPreview && typeof currentExpense?.attachment === 'string' && (
